...
export class SmsService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (accountSid && authToken && this.verifyServiceSid) {
      ...
      this.logger.warn(
        '⚠️ Twilio credentials not configured. SMS OTP will be logged to console only.',
      );
    }
  }
  ...
  async sendOtpSms(
    ...
    try {
      this.logger.log(
        `📱 Sending OTP to ${formattedPhone} via Twilio Verify...`,
      );
      ...
      if (
        ...
        this.devOtpStore.set(formattedPhone, otp);
        this.logger.warn(
          '⚠️ Twilio trial limitation - falling back to dev mode',
        );
        this.logger.warn(`📱 Use OTP: ${otp} for phone ${formattedPhone}`);
        return {
          success: true,
          message: 'OTP sent (dev mode - Twilio trial limitation, use 123456)',
        };
      }
      ...
      if (error.code === 60200) {
        ...
        return {
          success: false,
          message: 'Max send attempts reached. Please wait and try again.',
        };
      }
  ...
  async verifyOtp(
    ...
    if (this.devMode || !this.isConfigured || !this.twilioClient) {
      ...
      this.logger.warn(
        `[DEV MODE] Invalid OTP: ${code}, expected: 123456 or ${storedOtp}`,
      );
      return {
        success: false,
        message: 'Invalid OTP code. Use 123456 in dev mode.',
      };
    }
    ...
    try {
      ...
      if (verificationCheck.status === 'approved') {
        ...
        this.logger.warn(
          `❌ OTP verification failed: ${verificationCheck.status}`,
        );
      ...
      if (code === '123456' || code === storedOtp) {
        this.logger.warn(
          '⚠️ Twilio verification failed, using dev mode fallback',
        );
        this.devOtpStore.delete(formattedPhone);
